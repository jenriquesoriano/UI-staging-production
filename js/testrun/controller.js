ngApp.controller('myValidatorController', function($scope) {
	...
	$scope.readDataForm = function() {
		...
		var id = $_GET["id"];
		//$scope.testRunId = id.replace("EID", "");
		$scope.testRunId = id;
	}

	$scope.refreshStatusTest = function() {
		...
		$.getJSON($scope.urlValidator + "TestRuns/" + $scope.testRunId + ".json", function(data) {
			if (data.EtfItemCollection.testRuns !== undefined && data.EtfItemCollection.testRuns !== null) {
				...
				if ($scope.testResults[0].status == "UNDEFINED") $scope.testResults[0].status = "RUNNING";
				var statusAfterRun = $scope.testResults[0].status;
				console.log(statusAfterRun);
				$("#labelStatus").text(statusAfterRun);
				$("#backgroundStatus").removeClass("bg_undefined");
				$("#backgroundStatus").removeClass("bg_running");
				$("#backgroundStatus").removeClass("bg_UNDEFINED");
				$("#backgroundStatus").removeClass("bg_RUNNING");
				$("#backgroundStatus").addClass("bg_" + statusAfterRun);
				var extension = "png";
				if (statusAfterRun == "RUNNING") extension = "gif";
				$("#iconStatus").attr("src", "../img/icons/" + statusAfterRun + "." + extension);
				if ((statusAfterRun == "RUNNING") || (statusAfterRun == "UNDEFINED")) {
					setTimeout($scope.refreshStatusTest, 3000);
				}
			} else {
	...
	$scope.readTestResults = function() {
		...
		$scope.searchParameters.pageSize = 10;
	...
	$scope.showAllTestReports = function() {
		location.href = "../test-reports/index.html";
	}
